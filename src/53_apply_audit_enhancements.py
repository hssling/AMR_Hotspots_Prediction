"""
AMR Manuscript 3: Apply Audit Enhancements
Incorporates all peer review feedback to strengthen manuscript
"""
...
def apply_enhancements():
    """Apply all audit-recommended enhancements."""
    print("=" * 60)
    print("APPLYING AUDIT ENHANCEMENTS")
    print("=" * 60)
    
    # Load original manuscript
    doc_path = os.path.join(SUBMISSION_DIR, 'Manuscript_3_IJP_Main.docx')
    doc = Document(doc_path)
    
    # Track changes made
    changes = []
    
    # Find paragraphs and apply changes
    for i, para in enumerate(doc.paragraphs):
        text = para.text
        
        # 1. Fix Reference 9 - Correct author name
        if "Panda BK, Pati S, Palo SK" in text:
            ...
            changes.append("Fixed Reference 9: Corrected authors (Mathew P et al., not Panda BK et al.)")
        
        # 2. Add resistance definition clarification after data extraction sentence
        if "Data extraction was performed from official ICMR-AMRSN annual reports" in text:
            # Find the next paragraph and add clarification
            changes.append("Found Methods section - will add resistance definition clarification")
        
        # 3. Enhance Durbin-Watson statement in existing text
        if "the low Durbin-Watson statistic (0.56) suggests positive autocorrelation" in text.lower():
            new_text = text.replace(
                "The low Durbin-Watson statistic (0.56) suggests positive autocorrelation, which may inflate standard errors and affect statistical inference.",
            ...
            para.clear()
            para.add_run(new_text)
            changes.append("Enhanced Durbin-Watson autocorrelation warning")
    
    # Save enhanced version
    enhanced_path = os.path.join(SUBMISSION_DIR, 'Manuscript_3_IJP_Main_ENHANCED.docx')
    ...
    print(f"\nSaved enhanced manuscript: {enhanced_path}")
    
    return enhanced_path, changes

def create_enhanced_manuscript_v2():
    """Create fully enhanced manuscript with all audit feedback incorporated."""
    print("\n" + "=" * 60)
    print("CREATING FULLY ENHANCED MANUSCRIPT V2")
    print("=" * 60)
    
    # Read analysis results
    import json
    with open(os.path.join(BASE_DIR, "outputs", "its_analysis", "analysis_summary.json"), 'r') as f:
        results = json.load(f)
    
    import pandas as pd
    table1 = pd.read_csv(os.path.join(BASE_DIR, "outputs", "its_analysis", "table1_annual_trends.csv"))
    ...
    # ===================
    # INTRODUCTION (Enhanced with ESKAPE mention)
    # ===================
    ...
    # ===================
    # MATERIALS AND METHODS (Enhanced with resistance definition)
    # ===================
    ...
    # ENHANCED: Stronger Durbin-Watson caveat
    its_results_paras = [
    ...
    # ENHANCED: Added clinical significance discussion for A. baumannii
    sens_paras = [
    ...
    # ===================
    # DISCUSSION (Enhanced with ecological fallacy and stronger caveats)
    # ===================
    ...
    discussion_paras = [
        ...
        # ENHANCED: Added ecological fallacy statement
    ...
    # ===================
    # CONCLUSIONS (Enhanced with stronger caveat)
    # ===================
    h = doc.add_heading('Conclusions', level=1)
    h.runs[0].font.name = 'Times New Roman'
    
    # ENHANCED: Stronger conclusion with pre-intervention caveat
    ...
    # Save enhanced manuscript
    output_path = os.path.join(SUBMISSION_DIR, 'Manuscript_3_IJP_Main_ENHANCED_V2.docx')
    doc.save(output_path)
    print(f"\nEnhanced manuscript saved: {output_path}")
...
def main():
    ...
    print("=" * 70)
    print("AMR MANUSCRIPT 3: AUDIT ENHANCEMENT PIPELINE")
    print("=" * 70)
    
    # Create fully enhanced V2
    enhanced_path = create_enhanced_manuscript_v2()
    ...
    print("  1. ✅ Fixed Reference 9: Mathew P et al. (not Panda BK et al.)")
    print("  2. ✅ Added ESKAPE pathogens mention (Introduction)")
    print("  3. ✅ Added resistance definition clarification (Methods)")
    print("  4. ✅ Enhanced Durbin-Watson autocorrelation warning (Results)")
    print("  5. ✅ Added ecological fallacy statement (Discussion)")
    print("  6. ✅ Expanded A. baumannii clinical significance (Discussion)")
    print("  7. ✅ Strengthened pre-intervention data caveat (Conclusions)")
    print("  8. ✅ Added ARIMA recommendation for future studies (Discussion)")
